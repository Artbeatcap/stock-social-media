...
stock from the sent post-market telemetry. Appends a section to the daily
research log at validation_telemetry/reports/research_log_YYYY-MM-DD.md
"""
...
from validation_telemetry.store import load_daily_telemetry, reports_dir, session_date_et  # noqa: E402
...
def run_deep_dive(session_date: str | None = None) -> Path:
    ...
    news_24h = [n for n in news_7d if int(n.get("datetime") or 0) >= int((now - timedelta(hours=24)).timestamp())]
        ...
        if nearby_news:
            ...
            for item in nearby_news[:5]:
                section_lines.append(
                    f"  - {item.get('minutes_from_move')}m away: {item.get('headline') or item.get('title')}"
                )
        else:
    ...
    if not options.get("ok"):
        section_lines.append(f"- Options data unavailable: {options.get('error', 'entitlement or empty')}")
    elif unusual:
    ...
    log_path = reports_dir() / f"research_log_{session}.md"
    if log_path.exists():
        existing = log_path.read_text(encoding="utf-8").rstrip()
        if f"## Featured: ${ticker}" in existing:
            # Replace prior section for idempotent re-runs
            parts = existing.split(f"## Featured: ${ticker}")
