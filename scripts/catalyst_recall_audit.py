...
def audit_as_of_for_trade_date(trade_date: str) -> datetime:
    """Anchor the 24h news window to 9:15pm ET on the trade date (scheduled audit time)."""
    day = datetime.strptime(trade_date, "%Y-%m-%d").date()
    return NY.localize(datetime(day.year, day.month, day.day, 21, 15, 0))
...
def audit_mover(
    ...
    *,
    ...
    as_of = audit_as_of_for_trade_date(trade_date)
    now_ts = as_of.timestamp()
    headlines = get_news_for_ticker_window(ticker, hours=24, limit=20, as_of=as_of)[:5]
...
def run_audit(trade_date: Optional[str] = None) -> Path:
    ...
    for mover in gainers:
        if mover.get("ticker") or mover.get("symbol"):
            results.append(audit_mover(mover, "gainer", trade_date=trade_date))
    for mover in losers:
        if mover.get("ticker") or mover.get("symbol"):
            results.append(audit_mover(mover, "loser", trade_date=trade_date))
